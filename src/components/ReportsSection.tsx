...
const ReportsSection = () => {
      ...
      case 'sales':
        return (
          <div className="space-y-6">
            <Card className="bg-white/80 backdrop-blur-sm border-cafe-beige">
              <CardHeader><CardTitle className="flex items-center gap-2 text-cafe-brown-dark"><TrendingUp className="w-5 h-5" />تقرير المبيعات</CardTitle></CardHeader>
              <CardContent>
                <Table>
                  ...
                  <TableBody>{reports.salesReportData.map((item, index) => <TableRow key={index}><TableCell>{item.date}</TableCell><TableCell>{item.invoices}</TableCell><TableCell>{item.items}</TableCell><TableCell className="font-semibold text-cafe-brown">{item.total.toFixed(2)} د.ل</TableCell></TableRow>)}</TableBody>
                </Table>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"><h2 className="text-2xl font-bold text-cafe-brown-dark">التقارير والإحصائيات</h2></div>

      <Card className="bg-white/60 backdrop-blur-sm border-cafe-beige">
        <CardHeader><CardTitle className="flex items-center gap-2 text-cafe-brown-dark"><FileText className="w-6 h-6" />إعدادات التقرير</CardTitle></CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-2"><Label className="invisible">إجراءات</Label><Button onClick={generateReport} className="w-full bg-gradient-to-r from-cafe-brown to-[#7A3420]"><Calendar className="w-4 h-4 mr-2" />إنشاء التقرير</Button></div>
          </div>
        </CardContent>
      </Card>

      {noData ? <Card className="bg-white/80 backdrop-blur-sm border-cafe-beige"><CardContent className="py-12 text-center text-gray-500">لا توجد بيانات لهذا التقرير حالياً</CardContent></Card> : renderReportContent()}
    </div>
  );
};
