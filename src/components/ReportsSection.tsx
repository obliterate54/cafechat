            ...
            <Card className="bg-white/80 backdrop-blur-sm border-blue-100">
              ...
              <CardContent>
                <Table>
                  ...
                  <TableBody>{reports.salesReportData.map((item, index) => <TableRow key={index}><TableCell>{item.date}</TableCell><TableCell>{item.invoices}</TableCell><TableCell>{item.items}</TableCell><TableCell className="font-semibold text-blue-600">{item.total.toFixed(2)} ريال</TableCell></TableRow>)}</TableBody>
                </Table>
                <div className="h-72 mt-6">
                  <ResponsiveContainer width="100%" height="100%"><BarChart data={reports.salesReportData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis /><Tooltip formatter={(value) => [`${value} ريال`, 'المبيعات']} /><Bar dataKey="total" fill="#3B82F6" /></BarChart></ResponsiveContainer>
                </div>
              </CardContent>
            </Card>
          </div>
