  ...
  return (
    <div className="space-y-6">
      <Card className="bg-white/60 backdrop-blur-sm border-cafe-beige">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-cafe-brown-dark"><FileText className="w-6 h-6" />فواتير المبيعات</CardTitle>
          <p className="text-sm text-gray-600">إجمالي الفواتير: {salesInvoices.length} فاتورة</p>
        </CardHeader>
      </Card>

      <Card className="bg-white/60 backdrop-blur-sm border-cafe-beige">
        <CardHeader><CardTitle className="flex items-center gap-2 text-cafe-brown-dark"><Receipt className="w-5 h-5" />قائمة الفواتير ({salesInvoices.length})</CardTitle></CardHeader>
        <CardContent>
          {salesInvoices.length === 0 ? (
            ...
            <div className="space-y-4">
              {salesInvoices.map((invoice) => (
                <Card key={invoice.id} className="border-cafe-sand hover:shadow-md transition-all duration-200">
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto" dir="rtl">
          ...
          {selectedInvoice && (
            <div className="space-y-6">
              ...
              <div className="border-t pt-4"><div className="flex justify-between items-center text-xl font-bold"><span>المبلغ الإجمالي:</span><span className="text-cafe-brown">{selectedInvoice.total.toFixed(2)} د.ل</span></div></div>
              <div className="flex gap-2 pt-4 flex-wrap">
                <Button
                  className="flex-1 min-w-[180px] bg-gradient-to-r from-cafe-brown to-[#7A3420]"
                  onClick={async () => {
                    if (!selectedInvoice) return;

                    try {
                      await printSaleReceiptQz(selectedInvoice);
                    } catch (error) {
                ...
                <Button
                  variant="outline"
                  className="flex-1 min-w-[180px] border-cafe-sand hover:bg-cafe-cream"
                  onClick={() => {
                    if (!selectedInvoice) return;
                    printSaleReceipt(selectedInvoice);
                  }}
                >
                  <Printer className="w-4 h-4 mr-2" />
                  طباعة PDF / المتصفح
                ...
                <Button variant="destructive" onClick={() => handleDelete(selectedInvoice)}>
